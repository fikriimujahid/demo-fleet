  ...
  .map(({ label, href }) => {
    return (
      <li key={href} className="flex items-center">
        <Link href={href}>
          <a href="#pablo" className="bg-white text-blueGray-700 active:bg-blueGray-50 text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150">
            <small>{label}</small>
          </a>
        </Link>
      </li>
      ...
      <nav className="top-0 absolute z-50 w-full flex flex-wrap items-center justify-between px-2 py-3 navbar-expand-lg">
        <div className="container px-4 mx-auto flex flex-wrap items-center justify-between">
          ...
          >
            ...
            <ul className="flex flex-col lg:flex-row list-none lg:ml-auto">{links}</ul>
          </div>
        </div>
      </nav>
    </>
